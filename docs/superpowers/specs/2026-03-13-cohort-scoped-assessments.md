...
The existing enroll endpoint in `admin.py` always generates a Supabase invite link (JWT token) and sends the `user_enrolled` template, which includes an "Accept Invitation & Set Password" CTA. This is correct only for new users who have never activated their account.

### New behaviour

When a user is enrolled in a cohort, check whether they are already an **activated user** in Supabase Auth before deciding which email to send.

**Activation check:**
```python
user_data = supabase_admin.auth.admin.get_user(user_id)
is_activated = user_data.user.email_confirmed_at is not None
```

**Decision logic:**
```
if is_activated:
    → send cohort_enrollment_existing template (dashboard link, no JWT token)
else:
    → generate invite link, send user_enrolled template (activation link with JWT token)
```
...
No invite link is generated or sent for existing users.

### Where the check lives

In `app/routers/admin.py`, inside the enroll endpoint (both single-enroll and bulk-enroll paths). The logic replaces the current unconditional invite-link generation.
